...
import {
  getKokoroStatus, setupKokoro, downloadKokoroModel, speakWithKokoro,
  CURATED_KOKORO_VOICES, encodeKokoroVoiceSelection, decodeKokoroVoiceSelection,
  type KokoroStatus,
} from "../lib/kokoro";
...
import { supportsNativeTools, supportsCodeMode } from "../lib/modelCapabilities";
...
function DesktopIntegrationSection() {
  ...
  const { selectedVoiceName, setSelectedVoiceName, quickInvokeAutoSpeak, setQuickInvokeAutoSpeak } = useVoiceStore();
  ...
  function refreshKokoroStatus() {
    if (!tauri) return;
    void getKokoroStatus().then(setKokoroStatus).catch(() => setKokoroStatus(null));
  }
  ...
  useEffect(refreshKokoroStatus, [tauri]);

  async function handleKokoroSetup() {
    setKokoroBusy(true);
    try {
      const msg = await setupKokoro();
      toast.success(msg);
      refreshKokoroStatus();
    } catch (e) {
      toast.error(`Kokoro setup failed: ${(e as Error).message}`);
    } finally {
      setKokoroBusy(false);
    }
  }

  async function handleKokoroDownload() {
    setKokoroBusy(true);
    try {
      const msg = await downloadKokoroModel();
      toast.success(msg);
      refreshKokoroStatus();
    } catch (e) {
      toast.error(`Voice model download failed: ${(e as Error).message}`);
    } finally {
      setKokoroBusy(false);
    }
  }
  ...
  async function playVoiceSample() {
    ...
    try {
      const kokoroVoiceSel = decodeKokoroVoiceSelection(selectedVoiceName);
      if (kokoroVoiceSel) {
        await speakWithKokoro(VOICE_SAMPLE_TEXT, kokoroVoiceSel);
      } else {
        ...
        {tauri && (
          <div className="pt-3 border-t border-border space-y-2">
            ...
            <Field label="Voice">
              <div className="flex gap-2">
                ...
                >
                  ...
                  {kokoroStatus?.model_ready && (
                    <optgroup label="Kokoro (offline neural)">
                      {CURATED_KOKORO_VOICES.map((v) => (
                        <option key={v.id} value={encodeKokoroVoiceSelection(v.id)}>{v.label}</option>
                      ))}
                    </optgroup>
                  )}
                ...
                <Button
                  ...
                  disabled={samplePlaying || (voices.length === 0 && !kokoroStatus?.model_ready)}
            ...
            <label className="flex items-center gap-1.5 text-xs text-foreground cursor-pointer">
              <input
                type="checkbox"
                checked={quickInvokeAutoSpeak}
                onChange={(e) => setQuickInvokeAutoSpeak(e.target.checked)}
                className="accent-primary"
              />
              Speak Ctrl+Shift+K background answers aloud automatically
            </label>
            <p className="text-[11px] text-muted-foreground -mt-1">
              Only applies to plain-Enter (background widget) quick-invoke answers — Ctrl+Enter opens the full chat
              window, which already shows the answer, so it's unaffected.
            </p>
            ...
            <div className="pt-2 border-t border-border/60 space-y-1.5">
              ...
              {!kokoroStatus ? (
                ...
                <>
                  <p className="text-[11px] text-muted-foreground">
                    {kokoroStatus.python_available
                      ? `Not set up yet. This installs kokoro-onnx (Apache-2.0) into its own isolated Python environment under LocalMind's app data folder — nothing bundled into LocalMind itself. ${kokoroStatus.gpu_detected ? "An NVIDIA GPU was detected, so setup will install the GPU-accelerated onnxruntime package." : "No NVIDIA GPU was detected, so setup will install the CPU-only onnxruntime package."}`
                  ...
                  <Button
                    ...
                    size="sm"
                    className="gap-1.5"
                    disabled={kokoroBusy || !kokoroStatus.python_available}
                    onClick={() => void handleKokoroSetup()}
                  >
                    {kokoroBusy ? "Setting up… (can take a minute)" : "Set up Kokoro"}
                  </Button>
                </>
              ) : !kokoroStatus.model_ready ? (
                <>
                  <p className="text-[11px] text-muted-foreground">
                    Kokoro is set up — one more download gets the voice model (~90MB, covers all voices at once, no
                    per-voice downloads needed).
                  </p>
                  <Button
                    variant="outline"
                    size="sm"
                    className="gap-1.5"
                    disabled={kokoroBusy}
                    onClick={() => void handleKokoroDownload()}
                  >
                    {kokoroBusy ? "Downloading…" : "Download voice model"}
                  </Button>
                </>
              ) : (
                <p className="text-[11px] text-muted-foreground">
                  Ready — pick a Kokoro voice from the dropdown above.
                </p>
              )}
            </div>
          </div>
...
function PrivacySecuritySection() {
  ...
  const codeModeAvailable = !!selectedModel && supportsCodeMode(selectedModel);
            ...
            <div key={groupKey} className="space-y-1 pt-1 first:pt-0">
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                {groups.get(groupKey)!.map((def) =>
                  def.name === "run_tool_script" ? (
                    <label
                      key={def.name}
                      title={
                        codeModeAvailable
                          ? def.description
                          : `${def.description} — not offered to ${selectedModel || "the selected model"} regardless of this toggle: Code Mode is withheld from models judged unlikely to write a reliable script.`
                      }
                      className="col-span-2 flex items-start gap-1.5 text-xs text-foreground py-0.5 cursor-pointer"
                    >
                      <input
                        type="checkbox"
                      ...
                      <span>
                        ...
                        <span className="text-muted-foreground">
                          — batches a multi-step tool sequence into one script instead of one call per round.
                        </span>
                        {!codeModeAvailable && (
                          <span className="block text-[10px] text-muted-foreground italic mt-0.5">
                            Unavailable for {selectedModel || "the selected model"} — Code Mode is only offered to models judged capable of writing a reliable script.
                          </span>
                        )}
                      </span>
                    </label>
